...
describe(`DePayRouterV1ApproveAndCallContractAddressAmountBoolean01 on ${blockchain}`, function() {
  ...
  let addressWithBUSD = '0xe9e7CEA3DedcA5984780Bafc599bD69ADd087D56'
  ...
  it('deploys the contractCallPlugin', async () => {
    const Plugin = await ethers.getContractFactory('DePayRouterV1ApproveAndCallContractAddressAmountBoolean01')
  ...
  it('swaps NATIVE to BUSD and performs payment into smart contract in BUSD', async () => {
    ...
    await expect(
      router.connect(wallets[0]).route(
        ...
        ['stakeAddressAmountBooleanBUSD(address,uint256,bool)', 'true'], // data
        { value: amountIn }
      )
  ...
  it('swaps BUSD to BNB and performs payment into smart contract with BNB', async () => {
    ...
    await expect(
      router.connect(signer).route(
        ...
        [swapPlugin.address, contractCallPlugin.address], // plugins
        ['stakeAddressAmountBooleanNative(address,uint256,bool)', 'true'] // data
      )
