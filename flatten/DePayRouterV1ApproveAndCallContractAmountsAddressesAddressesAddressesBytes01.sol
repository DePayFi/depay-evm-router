...
// Root file: contracts/DePayRouterV1ApproveAndCallContractAmountsAddressesAddressesAddressesBytes01.sol
...
contract DePayRouterV1ApproveAndCallContractAmountsAddressesAddressesAddressesBytes {
  ...
  function execute(
    ...
    uint[] memory passedAmounts = new uint[](amounts[5]);
    for (uint i=0; i < amounts[5]; i++){
      passedAmounts.push(amounts[10+i]);
    }
    address[] memory passedAddresses1 = new address[](amounts[6]);
    for (uint i=0; i < amounts[6]; i++){
      passedAddresses1.push(addresses[2+i]);
    }
    address[] memory passedAddresses2 = new address[](amounts[7]);
    for (uint i=0; i < amounts[7]; i++){
      passedAddresses2.push(addresses[2+amounts[6]+1+i]);
    }
    address[] memory passedAddresses3 = new address[](amounts[8]);
    for (uint i=0; i < amounts[8]; i++){
      passedAddresses3.push(addresses[2+amounts[6]+amounts[7]+1+i]);
    }
    bytes[] memory passedBytes = new bytes[](amounts[9]);
    for (uint i=0; i < amounts[9]; i++){
      passedBytes.push(bytes(data[1+i]));
    }
